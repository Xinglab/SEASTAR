...
path <- argv[1]
  ...
  input <- as.matrix(read.delim(paste(path,"/tmp/RMATS_utr/rMATS_Result.txt",sep=""), header = TRUE))
...
#  input<-merge(ann,input,by='ID')
  ...
  write.table(input, paste(path,"/tmp/RMATS_utr/rMATS_utr_Result.txt",sep = ""),sep="\t", row.names=FALSE, col.name=TRUE, quote = FALSE)
  write.table(input, paste(path,"/rMATS_utr_Result.txt",sep = ""),sep="\t", row.names=FALSE, col.name=TRUE, quote = FALSE)
